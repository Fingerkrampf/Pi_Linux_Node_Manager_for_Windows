...
"""
Pi Linux Node Manager for Windows v1.01
...
Pi Linux Node Manager für Windows v1.01
Copyright (C) 2025 Fingerkrampf
...
translations = {
    "de": {
        "title": "Pi Linux Node Manager für Windows v1.01 von Fingerkrampf @ t.me/PiNetzwerkDeutschland",
        ...
        "upgrade_node": "Bestehenden Node Updaten",
    ...
    "en": {
        "title": "Pi Linux Node Manager for Windows v1.01 by Fingerkrampf @ t.me/PiNetzwerkDeutschland",
        ...
        "upgrade_node": "Update Pre-existing Node",
...
class PiNodeGUI(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        lang_frame = ctk.CTkFrame(sidebar_frame, fg_color="transparent")
        lang_frame.grid(row=5, column=0, padx=20, pady=10, sticky="s")
        self.lang_label = ctk.CTkLabel(lang_frame)
        self.lang_label.pack(side="left", padx=(0, 10))
        self.lang_menu = ctk.CTkOptionMenu(lang_frame, variable=self.current_lang, values=["de", "en"], width=80)
        ...
        # --- New System Status Frame ---
        self.system_status_frame = ctk.CTkFrame(main_frame, corner_radius=10)
        self.system_status_frame.grid(row=0, column=1, padx=(0, 20), pady=(20, 10), rowspan=2, sticky="nsew")
        ...
        self.log_label.grid(row=2, column=0, columnspan=2, padx=20, pady=(10, 5), sticky="w")
        self.log_text_frame = ctk.CTkFrame(main_frame, corner_radius=10)
        self.log_text_frame.grid(row=3, column=0, columnspan=2, padx=20, pady=(0, 20), sticky="nsew")
    ...
    def update_language(self, *args):
        ...
        self.title(t["title"])
        self.lang_label.configure(text=t["select_language"])
        
        self.conn_frame.configure(fg_color=("#dbdbdb", "#2B2B2B")) 
    ...
    def load_settings(self):
        try:
            with open(self.config_file, 'r') as f:
                ...
                self.current_lang.set(settings.get("lang", "de"))
    ...
    def disconnect(self):
        ...
        self.status_frame.configure(fg_color=("#dbdbdb", "#2B2B2B"))
        # Reset system stats labels
    ...
    def _get_prereq_script(self):
        ...
        echo "Ensuring Docker Compose v2 and cron are available..."
        sudo apt-get update
        sudo apt-get install -y docker-compose-plugin cron
    ...
    def _run_upgrade(self):
        ...
        upgrade_script = f"""
        set -e
        echo "--- Starting Upgrade Process based on official instructions ---"
        ...
        echo "--- Step 1: Collecting existing configuration values ---"
        NODE_SEED=""
        if [ -f mainnet.env ] && grep -qE '^(NODE_SEED|NODE_PRIVATE_KEY)=' mainnet.env; then
            NODE_SEED=$(grep -E '^(NODE_SEED|NODE_PRIVATE_KEY)=' mainnet.env | head -n 1 | cut -d '=' -f2)
        elif [ -f stellar-core.cfg ]; then
            NODE_SEED=$(grep -E 'NODE_SEED' stellar-core.cfg | sed -n 's/.*NODE_SEED="\\([^"]*\\)".*/\\1/p')
        fi
        
        POSTGRES_PASSWORD=$(grep -E '^POSTGRES_PASSWORD=' mainnet.env | cut -d '=' -f2)
        DOCKER_VOLUMES_PATH_RAW=$(grep '/var/lib/stellar' docker-compose.yml | sed -n 's/.*- *\\([^:]*\\):\\/var\\/lib\\/stellar.*/\\1/p' | xargs)

        if [ -z "$NODE_SEED" ] || [ -z "$POSTGRES_PASSWORD" ] || [ -z "$DOCKER_VOLUMES_PATH_RAW" ]; then
            echo "ERROR: Failed to automatically find all required values (seed, password, volume path)."
            exit 1
        fi

        if [[ "$DOCKER_VOLUMES_PATH_RAW" != /* ]]; then
            DOCKER_VOLUMES_PATH_ABS="$PROJECT_DIR/$DOCKER_VOLUMES_PATH_RAW"
        else
            DOCKER_VOLUMES_PATH_ABS="$DOCKER_VOLUMES_PATH_RAW"
        fi
        ...
        if [ -d "$DOCKER_VOLUMES_PATH_ABS" ]; then
            BACKUP_PATH="${{DOCKER_VOLUMES_PATH_ABS}}.backup-$(date +%F_%T)"
            echo "Creating backup at: $BACKUP_PATH"
            sudo cp -a "$DOCKER_VOLUMES_PATH_ABS" "$BACKUP_PATH"
        else
        ...
        pi-node initialize --pi-folder "$HOME/pi-node" --docker-volumes "$DOCKER_VOLUMES_PATH_ABS" --node-seed "$NODE_SEED" --postgres-password "$POSTGRES_PASSWORD" --auto-confirm --start-node
